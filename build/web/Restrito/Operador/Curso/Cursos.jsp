...
<html lang="pt">
    ...
    <body data-spy="scroll" data-target=".site-navbar-target" data-offset="300" onload="Resultado()">
        <div class="site-wrap">
            ...
            <div id="conteudo">
                ...
                <table class="table">
                    <thead class="thead-light"><tr><th></th><th>ID</th><th>Nome</th><th>Modalidade</th><th>Opções</th></tr></thead>
                    <tbody>
                        ...
                        <tr class="curso" id="<%=curso.getModalidade()%>">
                            <td></td>
                            <td><%=curso.getId()%></td>
                            <td><a href="/PWeb_SCA__DGJ_/Restrito/Operador/Curso/VerCurso.jsp?id=<%=curso.getId()%>"><%=curso.getNome()%></a></td>
                            <td><%=curso.getModalidade()%></td>
                            <td><a href="/PWeb_SCA__DGJ_/Restrito/Operador/Curso/AlterarCurso.jsp?id=<%=curso.getId()%>" class="btn btn-info">Alterar</a><button class="btn btn-danger" onclick="Confirmar(<%=curso.getId()%>, '<%=curso.getNome()%>')">Apagar</button></td>
                        </tr>
                        <%}%>
                    </tbody>
                </table>
    ...
    <script>
        ...
        function Confirmar(id, nome) {
            if (confirm("Deseja excluir o curso: " + nome + "?")) {
                window.location.replace("ExcluirCurso?id=" + id);
            }
        }
        function Curso() {
