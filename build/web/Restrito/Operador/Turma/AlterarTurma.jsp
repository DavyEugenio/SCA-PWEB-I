...
<html lang="pt">
        ...
        <div class="site-wrap">
            ...
            <div id="conteudo">
                ...
                <form action="../../../AlterarTurma" method="post">
                    <div class="container">
                        <input value="<%=turma.getId()%>" name="id" hidden>
                        <h2>Alterar Turma:</h2>
                        <label><b>ID:</b></label><label><%=turma.getId()%></label><br>
                        <label><b>Ano:</b></label>
                        <input type="number" name="ano" value="<%=turma.getAno()%>" max="2100" min="2010" required><br>
                        <label><b>Semestre:</b></label>
                        <select name="semestre">
                            <option class="semestre" value="1" selected=>1</option>
                            <option class="semestre" value="2">2</option>
                        </select><br>
                        <label><b>Turno:</b></label>
                        <select name="turno">
                            <option class="turno" value="Matutino" selected=>Matutino</option>
                            <option class="turno" value="Vespertino">Vespertino</option>
                            <option class="turno" value="Noturno">Noturno</option>
                        </select><br>
    ...
    <script type="text/javascript">
                                var semestre = document.getElementsByClassName("semestre"), turno = document.getElementsByClassName("turno"), curso = document.getElementsByClassName("curso");
                                for (var i = 0; i < semestre.length; i++) {
                                    if (semestre[i].value === "<%=turma.getSemestre()%>") {
                                        semestre[i].selected = true;
                                    } else {
                                        semestre[i].selected = false;
                                    }
                                }
                                for (var i = 0; i < turno.length; i++) {
                                    if (turno[i].value === "<%=turma.getTurno()%>") {
                                        turno[i].selected = true;
                                    } else {
                                        turno[i].selected = false;
                                    }
                                }
                                for (var i = 0; i < curso.length; i++) {
                                    if (curso[i].value === "<%=turma.getId_curso()%>") {
